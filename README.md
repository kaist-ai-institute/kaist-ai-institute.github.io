...
explanation:
  - management: #운영자문위원회
    - goal: "연구원 운영에 관한 주요사항에 대한 자문 및 심의" 
      roles: 
        - item: 연구원의 기본 운영계획 
        - item: 연구원 산하 연구조직의 설치, 폐쇄 
        - item: 기타 연구원 운영에 필요한 중요 사항
```

위원회에 대한 추가설명 (설치 목적, 기능 및 역할)은 `_data/members.yml`의 `explanation` 아래 해당하는 분류(e.g., 운영자문위원회)의 **goal, roles**부분에 작성해주시면 됩니다. 

roles (위원회의 기능 및 역할)에 해당하는 내용을 작성하시는 경우, 줄바꿈이 필요한 항목 별로 구분지어 `item`을 추가해주시면 됩니다.
